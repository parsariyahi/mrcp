...
class Encoder:
    def __init__(self, mapper: Optional[Dict[str, str]] = None, word_delimiter: Optional[str] = None, morse_word_delimiter: Optional[str] = None, char_delimiter: Optional[str] = None) -> None:
        """``Morse code`` Encoder, This class uses ``mrcp`` style for decoding the ``Morse code`` codes.

        Parameters
        ----------
        mapper : Dict[str: str], optional
            ``mapper`` is the main ``to_morse`` mapper, we use ``mrcp._translator.maps.TO_MORSE_MAPPER`` if you set it to ``None``, by default ``None``
